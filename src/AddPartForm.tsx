...
import { Camera, Sparkles, Loader2, ArrowRight, Check, AlertTriangle, Eye, ShoppingBag, User, X, Clock, RefreshCw } from "lucide-react";
        ...
        for (let i = 0; i < filesArray.length; i++) {
          ...
          while (attempts < 2 && !success) {
            ...
            try {
              ...
              const uploadPromise = uploadToCloudinary(blobToUpload);
              ...
              console.log("[AddPartForm] Successfully received downloadUrl from Cloudinary:", downloadUrl);
              success = true;
            } catch (err: any) {
              console.error(`[UPLOAD] attempt ${attempts} failed`, { message: err?.message, name: file.name, type: blobToUpload.type, size: blobToUpload.size });
              if (err?.message === 'upload/timeout') {
                setError(language === "bn" ? "আপলোড আটকে গেছে (Timeout)! আপনার ইন্টারনেট সংযোগ ধীর।" : "Upload timeout! Slow internet connection.");
                updateTask('error');
                break;
              } else {
                setError(language === "bn" ? `আপলোড ত্রুটি: ${err?.message}` : `Upload error: ${err?.message}`);
                updateTask('error');
                break;
              }
            }
          ...
          if (success && downloadUrl && typeof downloadUrl === 'string' && downloadUrl.startsWith('http')) {
            // Apply Cloudinary automatic format and quality if it's a cloudinary URL
            let finalUrl = downloadUrl;
            if (finalUrl.includes('res.cloudinary.com') && !finalUrl.includes('f_auto')) {
              const parts = finalUrl.split('/upload/');
              if (parts.length === 2) {
                finalUrl = `${parts[0]}/upload/f_auto,q_auto/${parts[1]}`;
              }
            }
            updateTask('success', finalUrl);
            console.log("[AddPartForm] Adding URL to images state:", finalUrl);
            setImages(prev => {
              const combined = [...prev, finalUrl];
              return combined.slice(0, 5);
            });
          } else if (success) {
            console.error("[AddPartForm] Upload seemed successful, but invalid URL returned:", downloadUrl);
            updateTask('error');
            setError(language === "bn" ? "আপলোডে সমস্যা হয়েছে। ছবিটির URL সঠিক নয়।" : "Upload failed due to invalid URL.");
          } else {
            updateTask('error');
            setError(prev => prev || (language === "bn" ? `"${file.name}" আপলোড ব্যর্থ হয়েছে! CORS বা App Check চেক করুন।` : `Upload failed for "${file.name}". Check CORS/App Check.`));
          }
        }
      } catch (err: any) {
    ...
    <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 md:p-7 border border-slate-150/80 dark:border-slate-800 shadow-xl relative overflow-hidden transition duration-300">
      ...
      <div className="grid lg:grid-cols-12 gap-8 mt-6">
        <div className="lg:col-span-7">
          {false ? (
        /* Locked profile prompt - disabled */
        ...
        <>
              ...
              <form onSubmit={handlePostListing} className="space-y-6">
                ...
                <div className="flex justify-center pt-4 pb-2">
                  <button
                    type="submit"
                    disabled={loading || uploadTasks.some(t => t.status !== 'success' && t.status !== 'error')}
                    className="w-full px-10 py-4.5 bg-gradient-to-r from-amber-500 to-orange-500 text-slate-950 hover:from-amber-600 hover:to-orange-600 disabled:opacity-50 disabled:cursor-not-allowed disabled:from-amber-700 disabled:to-orange-700 font-black rounded-xl text-sm transition-all flex items-center justify-center gap-2 shadow-xl shadow-amber-500/20 cursor-pointer"
                  >
                    {loading ? (
                      <Loader2 className="w-5 h-5 animate-spin text-slate-950" />
                    ) : uploadTasks.some(t => t.status !== 'success' && t.status !== 'error') ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin text-slate-950" />
                        <span>{language === "bn" ? "ছবি আপলোড হচ্ছে..." : "Uploading images..."}</span>
                      </>
                    ) : (
                      <>
                        <span>
