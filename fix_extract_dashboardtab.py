...
# ---------------------------------------------------------------
# 1) Create src/components/DashboardTab.tsx
#    ("My Dashboard" tab: stats cards, inventory/ads/admin/my-shop/
#     playstore-audit sub-tabs, order tracking desk. Extracted from App.tsx.
#     Pure presentational component - all state stays in App.tsx and is
#     passed down as props.)
# ---------------------------------------------------------------
dashboardtab_path = "src/components/DashboardTab.tsx"

dashboardtab_content = '''import { Suspense, lazy } from "react";
import {
...
os.makedirs(os.path.dirname(dashboardtab_path), exist_ok=True)
with open(dashboardtab_path, "w", encoding="utf-8") as f:
    f.write(dashboardtab_content)
changed_parts.append("dashboardtab-file")

# ---------------------------------------------------------------
# 2) src/App.tsx — add lazy import for DashboardTab (idempotent)
# ---------------------------------------------------------------
app_path = "src/App.tsx"
with open(app_path, "r", encoding="utf-8") as f:
    app_ts = f.read()

import_line = 'const DashboardTab = lazy(() => import("./components/DashboardTab"));'
old_import = 'const SellerShopPage = lazy(() => import("./components/SellerShopPage").then(m => ({ default: m.SellerShopPage })));'
...
# ---------------------------------------------------------------
# 3) src/App.tsx — replace inline Dashboard-tab JSX with <DashboardTab />
# ---------------------------------------------------------------
start_marker = "            {activeTab === \'my-dashboard\' && user && ("
end_marker = "            {/* TAB: CHAT / MESSAGES */}"

if "<DashboardTab" in app_ts:
    changed_parts.append("app-jsx-already-present")
else:
    start_idx = app_ts.find(start_marker)
    end_idx = app_ts.find(end_marker)
    if start_idx == -1 or end_idx == -1 or start_idx >= end_idx:
        print("[WARN] Dashboard-tab block markers not found — no JSX changes made, check src/App.tsx manually")
    else:
        new_block = (
...
# Summary
# ---------------------------------------------------------------
print("=== fix_extract_dashboardtab.py রেজাল্ট ===")
print("Patched sections:", changed_parts)

ok_fresh = {"dashboardtab-file", "app-import", "app-jsx"}
ok_rerun = {"dashboardtab-file", "app-import-already-present", "app-jsx-already-present"}
got = set(changed_parts)

if ok_fresh.issubset(got) or ok_rerun.issubset(got):
    print("[OK] Dashboard tab \u09b8\u09ab\u09b2\u09ad\u09be\u09ac\u09c7 src/components/DashboardTab.tsx-\u098f extract \u09b9\u09af\u09bc\u09c7\u099b\u09c7, App.tsx \u098f\u0996\u09a8 \u0986\u09b0\u0993 \u099b\u09cb\u099f।")
else:
