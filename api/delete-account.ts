...
async function resolveCallerUid(token: string): Promise<{ authUid: string; firebaseUid?: string } | null> {
  if (supabaseAdmin) {
    try {
  ...
  if (getApps().length) {
    try {
      ...
      return { authUid: decoded.uid, firebaseUid: decoded.uid };
    } catch (e) {
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const { authUid, firebaseUid } = resolved;
