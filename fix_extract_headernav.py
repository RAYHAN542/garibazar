...
# ---------------------------------------------------------------
# 1) Create src/components/HeaderNav.tsx
#    (Header desktop nav + mobile bottom nav dock, extracted from App.tsx.
#     Pure presentational component - no state of its own, everything
#     comes in as props from App.tsx.)
# ---------------------------------------------------------------
headernav_path = "src/components/HeaderNav.tsx"

headernav_content = '''import { Grid, ShoppingBag, Plus, History, MessageSquare, User } from "lucide-react";
...
os.makedirs(os.path.dirname(headernav_path), exist_ok=True)
with open(headernav_path, "w", encoding="utf-8") as f:
    f.write(headernav_content)
changed_parts.append("headernav-file")

# ---------------------------------------------------------------
# 2) src/App.tsx — add import for HeaderNav (idempotent)
# ---------------------------------------------------------------
app_path = "src/App.tsx"
with open(app_path, "r", encoding="utf-8") as f:
    app_ts = f.read()

import_line = 'import { HeaderNav } from "./components/HeaderNav";'
old_import = 'import { ListingCard } from "./components/ListingCard";'
...
if import_line in app_ts:
    ...
    print("[WARN] import anchor not found in App.tsx — check imports manually")

# ---------------------------------------------------------------
# 3) src/App.tsx — replace inline Header+BottomNav JSX with <HeaderNav />
# ---------------------------------------------------------------
start_marker = "      {/* 1. Header & Navigation Panel */}"
end_marker = "      {/* 4. MAIN LAYOUT AND CORE VIEWS */}"

if "<HeaderNav" in app_ts:
    changed_parts.append("app-jsx-already-present")
else:
    start_idx = app_ts.find(start_marker)
    end_idx = app_ts.find(end_marker)
    if start_idx == -1 or end_idx == -1 or start_idx >= end_idx:
        print("[WARN] Header/BottomNav block markers not found — no JSX changes made, check src/App.tsx manually")
    else:
        new_block = (
            "      <HeaderNav\n"
            "        language={language}\n"
...
# Summary
# ---------------------------------------------------------------
print("=== fix_extract_headernav.py রেজাল্ট ===")
print("Patched sections:", changed_parts)

ok_fresh = {"headernav-file", "app-import", "app-jsx"}
ok_rerun = {"headernav-file", "app-import-already-present", "app-jsx-already-present"}
got = set(changed_parts)

if ok_fresh.issubset(got) or ok_rerun.issubset(got):
    print("[OK] Header + Bottom Nav সফলভাবে src/components/HeaderNav.tsx-এ extract হয়েছে, App.tsx এখন ছোট।")
else:
