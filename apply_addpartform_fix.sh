  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err?.code === "permission-denied") {
        // firestore.rules rejects for more than one reason (cooldown, bad
        // field, uid mismatch, etc.) but only reports "permission-denied"
        // either way -- so before blaming the cooldown, actually check the
        // user's own rate-limit doc (they're always allowed to read it) to
        // see whether they're really still inside the 30s window.
        let isRealCooldown = false;
        let secondsLeft = 30;
        try {
          const userLimitsRef = doc(db, "userLimits", currentUser.uid);
          const snap = await getDoc(userLimitsRef);
          const lastAt = snap.exists() ? snap.data()?.lastListingCreatedAt : null;
          if (lastAt?.toMillis) {
            const elapsed = (Date.now() - lastAt.toMillis()) / 1000;
            if (elapsed < 30) {
              isRealCooldown = true;
              secondsLeft = Math.ceil(30 - elapsed);
            }
          }
        } catch (checkErr) {
          console.error("Cooldown check failed:", checkErr);
        }

        if (isRealCooldown) {
          setError(
            language === "bn"
              ? `আপনি একটু আগেই একটা বিজ্ঞাপন পোস্ট করেছেন। অনুগ্রহ করে ${secondsLeft} সেকেন্ড অপেক্ষা করে আবার চেষ্টা করুন।`
              : `You just posted a listing. Please wait ${secondsLeft} more seconds and try again.`
          );
        } else {
          // Genuinely unexpected permission error -- don't lie and blame
          // the cooldown, since that hides real bugs (stale login, uid
          // mismatch, etc.) behind a message that tells the user to just
          // wait, which will never fix it.
          setError(
            language === "bn"
