...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Guard: gari_bazar_session_user (localStorage, UI-only cache) can outlive
    // the real Firebase Auth session (e.g. old cache from before switching to
    // browserLocalPersistence, a revoked/expired token, cache copied across
    // devices, etc). When that happens the UI looked "logged in" while every
    // auth-protected Firestore read (chats, admin analytics) silently came
    // back empty. Once Firebase reports its real state, clear the stale local
    // cache if there's no matching real session, so the user is prompted to
    // log in again instead of seeing a broken "ghost" logged-in UI.
    const unsubscribe = onAuthStateChanged(auth, (fbUser) => {
      if (!fbUser && localStorage.getItem("gari_bazar_session_user")) {
