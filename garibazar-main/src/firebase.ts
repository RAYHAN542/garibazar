...
// (Firebase Auth-এর ডিফল্ট persistence পদ্ধতি, যা IndexedDB ব্যবহার করে), যার
// ফলে sign-in "Database is closing/hidden" জাতীয় error দিয়ে ব্যর্থ হয়।
// browserLocalPersistence প্লেইন window.localStorage ব্যবহার করে — তাই সেই
// IndexedDB সমস্যাও এড়ানো যায় এবং একইসাথে localStorage ব্রাউজার/অ্যাপ বন্ধ
// করার পরও টিকে থাকে, ফলে লগইন সেশন বহাল থাকে। (আগে browserSessionPersistence
// ব্যবহার করা হতো, যা sessionStorage-ভিত্তিক — ব্রাউজার ট্যাব/অ্যাপ বন্ধ হলেই
// আসল Firebase Auth সেশন হারিয়ে যেত, যদিও App.tsx-এর নিজস্ব localStorage
// ক্যাশ ["gari_bazar_session_user"] তখনও ইউজারকে "লগইন করা আছে" দেখাতো —
// এর ফলে Chat/Admin-analytics-এর মতো auth-protected Firestore read গুলো
// নিঃশব্দে permission-denied হয়ে খালি রেজাল্ট দেখাতো।)
setPersistence(auth, browserLocalPersistence).catch((err) => {
  logger.debug("Failed to set browserLocalPersistence, using default:", err);
});
