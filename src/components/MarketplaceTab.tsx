...
export default function MarketplaceTab({
  ...
  return (
              <div>
                ...
                {/* Reset/All Categories chip — its own thin line, only when a filter
                    is actually active, so the compact row above stays uncluttered
                    in the common (no filter) case. */}
                {(selectedCategory !== "all" || selectedSubCategory !== "all" || selectedCity !== "all") && (
                ...
                {/* 🚀 Search bar on top with history caching & Filter Tune Button (desktop) */}
            ...
            >
              ...
              {selectedCategory === "spare_parts" && (
                ...
                {listings.length === 0 ? (
                  ...
                  // Still might have matches further back that we haven't fetched yet
                  // (App.tsx auto-fetches them in the background) - show a loading
                  // state instead of "no matches" so visitors don't leave thinking
                  // nothing exists under this filter.
