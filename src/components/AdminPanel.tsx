...
export function AdminPanel({ language, currentUser, listings: listingsProp, isUserAdmin }: AdminPanelProps) {
  ...
  const refreshAnalytics = async () => {
    ...
    await Promise.all([fetchAnalyticsStats(), fetchSiteVisits()]);
  ...
  const loadMoreRefillRequests = async () => {
    ...
    try {
      ...
      setRefillLoadingMore(false);
    }
  };
  // TODO (Jesmin): refill_requests-এর তালিকা যেখানে JSX-এ রেন্ডার হয়, তার নিচে
  // {refillHasMore && <button onClick={loadMoreRefillRequests}>আরও দেখুন</button>}
  // বসিয়ে দিন - state/handler এখানে রেডি করে রাখা হলো, শুধু বাটনটা UI-তে বসানো বাকি।
      ...
      setActionSuccessMsg(
        language === "bn" 
          ? (request.type === "ad_promotion" 
              ? `সফলভাবে বিজ্ঞাপন প্রমোশন রিকোয়েস্ট অনুমোদিত হয়েছে!` 
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800 gap-1.5 pt-1 overflow-x-auto [-webkit-overflow-scrolling:touch] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        >
          {language === "bn" ? `পেমেন্ট রিকোয়েস্টস (${pendingRequestsCount})` : `Payment Requests (${pendingRequestsCount})`}
        </button>
      ...
      {adminSubTab === "settings" ? (
          ...
          <form onSubmit={handleUpdatePaymentConfig} className="space-y-4">
            ...
            {saveSuccess && (
              <p className="text-emerald-500 text-xs font-bold flex items-center gap-1.5 font-sans">
                ...
                {language === "bn" ? "মোবাইল নম্বরসমূহ সফলভাবে আপডেট করা হয়েছে!" : "Payment details saved successfully! All users will see these numbers."}
              </p>
            )}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 p-4 rounded-2xl">
            <div>
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold mt-0.5">
                {language === "bn" ? "যেকোনো পোস্ট মার্কেটপ্লেস থেকে সরাসরি মুছে ফেলে শৃঙ্খলা বজায় রাখুন।" : "Audit active posts on the hub. Delete any inappropriate listings or fraud attempts instantly."}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative max-w-xs w-full shrink-0">
                ...
                <input
                  ...
                  placeholder={language === "bn" ? "পার্টস বা আইডি দিয়ে খুঁজুন..." : "Search parts or advertiser..."}
              ...
              return (
                <div className="bg-slate-50 dark:bg-slate-950 border border-dashed rounded-2xl p-12 text-center text-slate-500">
                  ...
                  <p className="text-xs font-medium">
                    {language === "bn" ? "কোন লিস্টিং পাওয়া যায় নি!" : "No active listings found matching your search query."}
                  </p>
                </div>
              );
            }
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="pt-2 border-t border-slate-100 dark:border-slate-850 space-y-1 text-[11px] font-sans">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="text-slate-700 dark:text-slate-300 font-black">{item.sellerName}</span>
                        </div>
                        <div className="flex justify-between">
        ...
        <div className="space-y-4">
          <h5 className="text-sm font-black text-slate-850 dark:text-white mb-2 flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] font-bold uppercase rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-300 disabled:opacity-50"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${analyticsRefreshing ? "animate-spin" : ""}`} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl text-center">
              ...
              <p className="text-lg font-black text-slate-850 dark:text-white">{(analyticsStats.totalInstalls || 0).toLocaleString()}</p>
              <p className="text-[9px] text-slate-400 font-bold uppercase">{language === "bn" ? "অ্যাপ ইনস্টল" : "App Installs"}</p>
            </div>
          </div>
        ...
        <div className="space-y-4">
          ...
          {loadingRequests ? (
            ...
            <div className="bg-slate-50 dark:bg-slate-950 border border-dashed rounded-2xl p-10 text-center text-slate-500">
              ...
              <p className="text-xs font-medium">{language === "bn" ? "কোনো রিচার্জের আবেদন পাওয়া যায়নি" : "No budget refill requests submitted yet."}</p>
            </div>
          ) : (
                    ...
                    <div className="p-3 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 rounded-xl space-y-2 font-sans">
                      {req.type === "ad_promotion" ? (
                        <>
                          ...
                          <div className="flex justify-between items-center text-[11px]">
                            <span className="text-slate-400">
                              {language === "bn" ? "মেয়াদকাল:" : "Ad Duration:"}
                            </span>
