...
// Facebook, Instagram, Messenger, and similar apps open links in their own
// embedded in-app WebView, not real Chrome. Google actively refuses OAuth
// sign-in inside these embedded WebViews (its own anti-phishing policy,
// "disallowed_useragent") — no amount of app-side code can make Google's
// popup succeed there, it just hangs until our timeout fires with a
// confusing "Tracking Protection" message. Detecting this and steering the
// user straight to the Phone-number login (which works fine everywhere) is
// what production apps do instead of fighting a losing battle with the popup.
const isInAppBrowser = (): boolean => {
  if (typeof navigator === "undefined") return false;
  const ua = navigator.userAgent || "";
  return /FBAN|FBAV|FB_IAB|FBSV|Instagram|Line\/|MicroMessenger|Messenger/i.test(ua);
};
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full shadow-2xl p-6 relative">
        ...
        {step === "start" ? (
          <div className="space-y-3">
            ...
            {isInAppBrowser() ? (
              <div className="p-3 bg-amber-500/10 text-amber-700 dark:text-amber-400 rounded-lg text-xs text-center">
                {language === "bn"
                  ? "এই অ্যাপের ভেতরের ব্রাউজারে Google সাইন-ইন কাজ করে না (Google-এর নিজস্ব নিরাপত্তা নীতি)। নিচে মোবাইল নম্বর দিয়ে চালিয়ে যান, অথবা লিংকটি Chrome ব্রাউজারে খুলুন।"
                  : "Google sign-in doesn't work inside this app's built-in browser (a Google security restriction). Please continue with your mobile number below, or open this link in Chrome."}
              </div>
            ) : (
              <button
                type="button"
