...
# ---------------------------------------------------------------
# 1) Create src/components/MarketplaceTab.tsx
#    (Search bar, category/city/sort filters, promoted-ads slider,
#     listing grid + load-more, PWA install prompt, notification-
#     permission prompt. Extracted from App.tsx.)
# ---------------------------------------------------------------
markettab_path = "src/components/MarketplaceTab.tsx"

markettab_content = '''import { Search, SlidersHorizontal, Bell, Check, Plus, X, ShoppingBag, Loader2 } from "lucide-react";
...
os.makedirs(os.path.dirname(markettab_path), exist_ok=True)
with open(markettab_path, "w", encoding="utf-8") as f:
    f.write(markettab_content)
changed_parts.append("markettab-file")

# ---------------------------------------------------------------
# 2) src/App.tsx — remove the category constants now living in
#    MarketplaceTab.tsx (idempotent: only removes if still present)
# ---------------------------------------------------------------
app_path = "src/App.tsx"
with open(app_path, "r", encoding="utf-8") as f:
    app_ts = f.read()

old_consts = '''const VEHICLE_SUBCATEGORIES = [
  { id: "all", bnName: "সব গাড়ি", enName: "All Vehicles" },
  { id: "excavator", bnName: "এক্সক্যাভেটর", enName: "Excavator" },
  { id: "crane", bnName: "ক্রেন", enName: "Crane" },
  { id: "car", bnName: "কার", enName: "Car" },
...
new_consts = 'const checkIsProduction = (): boolean => {'

if old_consts in app_ts:
    app_ts = app_ts.replace(old_consts, new_consts, 1)
    changed_parts.append("app-consts-removed")
elif new_consts in app_ts and "VEHICLE_SUBCATEGORIES" not in app_ts:
    changed_parts.append("app-consts-already-removed")
else:
    print("[WARN] category-constants anchor not found as expected — check src/App.tsx lines ~85-115 manually")

# ---------------------------------------------------------------
# 3) src/App.tsx — add lazy import for MarketplaceTab (idempotent)
# ---------------------------------------------------------------
import_line = 'const MarketplaceTab = lazy(() => import("./components/MarketplaceTab"));'
old_import = 'const DashboardTab = lazy(() => import("./components/DashboardTab"));'

if import_line in app_ts:
    changed_parts.append("app-import-already-present")
elif old_import in app_ts:
    app_ts = app_ts.replace(old_import, old_import + "\n" + import_line, 1)
    changed_parts.append("app-import")
else:
    print("[WARN] lazy-import anchor not found in App.tsx — check imports manually")

# ---------------------------------------------------------------
# 4) src/App.tsx — replace inline Marketplace-tab JSX with <MarketplaceTab />
# ---------------------------------------------------------------
start_marker = "            {activeTab === \'market\' && ("
end_marker = "            {/* TAB 2: POST / SELL */}"

if "<MarketplaceTab" in app_ts:
    changed_parts.append("app-jsx-already-present")
else:
    start_idx = app_ts.find(start_marker)
    end_idx = app_ts.find(end_marker)
    if start_idx == -1 or end_idx == -1 or start_idx >= end_idx:
        print("[WARN] Marketplace-tab block markers not found — no JSX changes made, check src/App.tsx manually")
    else:
        new_block = (
            "            {activeTab === \'market\' && (\n"
        ...
        app_ts = app_ts[:start_idx] + new_block + app_ts[end_idx:]
        changed_parts.append("app-jsx")
...
# Summary
# ---------------------------------------------------------------
print("=== fix_extract_marketplacetab.py রেজাল্ট ===")
print("Patched sections:", changed_parts)

ok_fresh = {"markettab-file", "app-consts-removed", "app-import", "app-jsx"}
ok_rerun = {"markettab-file", "app-consts-already-removed", "app-import-already-present", "app-jsx-already-present"}
got = set(changed_parts)

if ok_fresh.issubset(got) or ok_rerun.issubset(got):
    print("[OK] Marketplace tab \u09b8\u09ab\u09b2\u09ad\u09be\u09ac\u09c7 src/components/MarketplaceTab.tsx-\u098f extract \u09b9\u09af\u09bc\u09c7\u099b\u09c7, App.tsx \u098f\u0996\u09a8 \u0986\u09b0\u0993 \u099b\u09cb\u099f।")
else:
    print(f"[PARTIAL] \u098f\u0987 \u0985\u0982\u09b6\u0997\u09c1\u09b2\u09cb miss \u09b9\u09af\u09bc\u09c7\u099b\u09c7/\u099a\u09c7\u0995 \u0995\u09b0\u09c1\u09a8: {(ok_fresh - got)}")
